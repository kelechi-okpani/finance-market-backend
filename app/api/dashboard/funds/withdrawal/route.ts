...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // --- Step 1: Fetch verified resettlement accounts ---
        ...
        if (userResettlementAccounts.length === 0) {
            return corsResponse({ 
                error: "No verified resettlement account found.",
                helpText: "You must first create a verified account through the 'Apply for Resettlement' section."
            }, 400, origin);
        }

        // --- Step 2: Validate submitted details ---
        let settlementAccount: any;
        ...
        if (resettlementAccountId) {
            settlementAccount = userResettlementAccounts.find(
                acc => acc._id.toString() === resettlementAccountId.toString()
            );
        }

        // Fallback: Find by account number and bank name matching
        ...
        if (!settlementAccount) {
            return corsResponse({ 
                error: "The bank details do not match your verified accounts.",
                verifiedAccountsSummary: userResettlementAccounts.map(a => 
        ...
        const cashMovement = await CashMovement.create({
            ...
            type: "withdrawal",
            amount: numAmount,
            currency: currency || user.baseCurrency || "USD",
            method: "resettlement",
            status: "pending",
            description: description || "Wallet Withdrawal to Resettlement Account",
            settlementAccountId: settlementAccount._id,
            date: new Date().toISOString() // Pass full ISO string to match your model fix
        });
        ...
        return corsResponse({
            success: true,
            message: "Withdrawal request submitted. Funds have been locked for processing.",
            cashMovement,
            settlementAccount: {
