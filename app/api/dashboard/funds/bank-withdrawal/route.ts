...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const failedAttempts = user.failedWithdrawalAttempts || 0;
        if (failedAttempts >= 3) {
            return corsResponse({
                error: "Withdrawal access restricted due to multiple failed attempts. Contact support.",
                requiresResettlement: true,
                failedAttempts
            }, 403, origin);
        }

        // --- Step 1: Verify Settlement Account ---
        const userResettlementAccounts = await SettlementAccount.find({
            userId: auth.user!._id,
            status: "verified"
        });

        let settlementAccount = userResettlementAccounts.find(acc => {
            if (settlementAccountId && acc._id.toString() === settlementAccountId.toString()) return true;
            if (accountNumber && acc.accountNumber === accountNumber) return true;
            return false;
        });

        if (!settlementAccount) {
            return corsResponse({ 
                error: "Verified resettlement account not found or details mismatch.",
                verifiedAccounts: userResettlementAccounts.map(a => `${a.bankName} (***${String(a.accountNumber).slice(-4)})`)
            }, 400, origin);
        }

        // Balance Check
        if (user.availableCash < numAmount) {
            return corsResponse({ error: "Insufficient available cash." }, 400, origin);
        }
        ...
        const bankPayload = {
            ...
            amount: numAmount,
            narration: narration || "Direct withdrawal",
            sendersName: "Apple Inc" 
        };
        ...
        if (!bankData.success) {
            ...
            return corsResponse({ 
                error: bankData.message || "Bank API declined the transaction.",
                failedAttempts: user.failedWithdrawalAttempts
            }, 400, origin);
        }

        // --- Step 3: Success - Update Database ---
        // 1. Create CashMovement (Completed)
        const cashMovement = await CashMovement.create({
            ...
            status: "completed",
            description: `Bank Withdrawal to ${settlementAccount.bankName}`,
        ...
        await Transaction.create({
            ...
            type: "withdrawal",
            amount: numAmount,
            description: `Withdrawal Ref: ${bankData.data?.reference || 'N/A'}`,
            status: "completed"
        });

        // 3. Update User Balance & Reset Strikes
        ...
        return corsResponse({ error: "Internal server error during processing." }, 500, origin);
    }
}
