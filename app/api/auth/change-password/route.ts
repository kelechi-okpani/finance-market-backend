...
export async function PUT(request: NextRequest) {
    ...
    try {
        ...
        if (!currentPassword || !newPassword) {
            return corsResponse({ error: "Current and new passwords are required." }, 400, origin);
        }
        ...
        if (!isMatch) {
            return corsResponse({ error: "Incorrect current password." }, 401, origin);
        }
        ...
        return corsResponse({ error: "Failed to change password.", details: error.message }, 500, origin);
    }
}
