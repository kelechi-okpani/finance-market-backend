...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!email) {
            return corsResponse({ status_code: 400, message: "Email is required." }, 400, origin);
        }

        await connectDB();
        const emailLower = email.toLowerCase().trim();

        // 2. Check AccountRequest Status (Matching your existing logic)
        const accountReq = await AccountRequest.findOne({ email: emailLower });
        if (accountReq && accountReq.status !== "approved") {
            return corsResponse({
                status_code: 404,
                message: `Your account status is currently ${accountReq.status}. You can reset your password once it is approved.`,
            }, 404, origin);
        }

        // 3. Find User & Apply Updates (Matching Model Fields)
        ...
        // CRITICAL: Verify these match your User.ts schema
        ...
        // 4. Send the email using firstName from the model
        // If your model uses 'first_name', change user.firstName to user.first_name
        ...
        if (!mailResult.success) {
            return corsResponse({
                ...
                details: (mailResult.error as any)?.message
            }, 500, origin);
        }
        ...
        return corsResponse({
            ...
            ...(isSimulated && { dev_simulated_otp: otpCode }) // Helpful for development
        }, 200, origin);
...
//             return corsResponse({
//                 status_code: 404,
//                 message: `Your account request is currently not ${accountReq.status}. You will be able to set your password once it is approved.`,
